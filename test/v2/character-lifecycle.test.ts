  ...
  it("converts core statistics, proficiencies, inventory and spell slots into the migration contract", async () => {
    const ddb = {
      data: {
        ...
        race: { baseName: "Elf", subRaceShortName: "High Elf", weightSpeeds: { normal: { walk: 30 } } },
    ...
    expect(character.identity).toMatchObject({
      className: "Wizard", subclass: "Evocation", species: "Elf", subrace: "High Elf", background: "Sage", level: 3,
    });
