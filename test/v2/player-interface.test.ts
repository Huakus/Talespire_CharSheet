...
describe("player interface shell", () => {
  it("starts as a compact play sheet with stable section navigation", () => {
    ...
    expect(html).toContain('<details class="color-picker menu-color-picker"><summary title="Cambiar color del personaje"><span>Color</span>');
    ...
    expect(html).not.toContain("Último log");
    expect(html).toContain('class="connection-indicators"');
    expect(html).toContain('data-tooltip="Comprobando servidor externo…"');
    expect(html).toContain('<svg viewBox="0 0 24 24"');
    expect(html).toContain("data-open-persistence");
    ...
    character.identity.className = "Mago";
    const configured = view.renderCharacterForm(character);
    expect(configured).toContain('class="character-fact" aria-label="Clase: Mago">Mago</span>');
    expect(configured).not.toContain('aria-label="Clase: Mago">Clase</span>');

    view.sheetMode = "edit";
    const editor = view.renderCharacterForm(character);
    for (const field of ["className", "subclass", "species", "subrace", "background", "alignment"]) {
  ...
  it("shows every session message but counts only unread errors", () => {
    const view = harness();
    view.notifications = [
      { id: 1, kind: "success", text: "Guardado correctamente", occurredAt: "2026-08-05T12:00:00.000Z" },
      { id: 2, kind: "error", text: "Conflicto remoto", occurredAt: "2026-08-05T12:01:00.000Z" },
    ];
    view.unreadImportantNotifications = 1;

    const html = view.renderNotificationCenter();
    expect(html).toContain("Guardado correctamente");
    expect(html).toContain("Conflicto remoto");
    expect(html).toContain('class="notification-badge">1</strong>');
    expect(html).toContain('class="notification-center has-unread"');
    expect(html).toContain('<svg viewBox="0 0 24 24"');
  });

  it("describes changed fields and their new values for the action log", () => {
    const before = createCharacter("chr_22222222222222222222222222222222", "Hero", "2026-08-05T00:00:00.000Z");
    const after = structuredClone(before);
    after.identity.background = "Soldado";
    after.combat.hitPoints.current = 7;
    expect(describeCharacterChanges(before, after)).toEqual([
      "Identidad › Trasfondo → “Soldado”",
      "Combate › Puntos de golpe › Actual → 7",
    ]);
  });
  ...
  it("renders compact hit-point controls and the three inspiration states", () => {
    ...
    expect(inactive.indexOf('class="ability-cards"')).toBeLessThan(inactive.indexOf('class="passive-score-strip"'));
    expect(inactive.indexOf('class="passive-score-strip"')).toBeLessThan(inactive.indexOf('class="checks-roll-layout"'));
    expect(inactive.indexOf('class="checks-roll-layout"')).toBeLessThan(inactive.indexOf('class="condition-toggle-panel"'));
    expect(inactive).toContain('aria-label="Velocidad: 30 ft">30 ft</span>');
    ...
    expect(inactive.indexOf('class="death-save-control"')).toBeLessThan(inactive.indexOf('class="character-facts"'));
