...
export class GmToolsPanel {
  ...
  private renderSpellForm(spell: SpellDefinition | null): string {
    ...
    return `<form data-gm-form="spell" class="gm-editor-form">
      ...
      <div class="catalog-editor-meta"><span class="catalog-origin ${escapeHtml(meta.origin)}">${escapeHtml(meta.origin)}</span><label>Etiquetas de campaña<input name="catalogTags" value="${escapeHtml(meta.tags.join(", "))}" placeholder="oficial, reglas, fuego"></label></div>
  ...
  private renderEquipmentForm(item: EquipmentCatalogDraft | null): string {
    ...
    return `<form data-gm-form="equipment" class="gm-editor-form"><input type="hidden" name="previousKey" value="${escapeHtml(item?.name ?? "")}">
      <div class="catalog-editor-meta"><span class="catalog-origin ${escapeHtml(meta.origin)}">${escapeHtml(meta.origin)}</span><label>Etiquetas de campaña<input name="catalogTags" value="${escapeHtml(meta.tags.join(", "))}" placeholder="oficial, tesoro, mágico"></label></div>
