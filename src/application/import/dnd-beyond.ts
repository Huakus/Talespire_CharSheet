...
export function convertDndBeyondCharacter(input: unknown): { characters: Record<string, unknown> } {
  ...
  const legacy = {
    ...
    playerSubclass: classes(character).map((entry) => String(record(entry.subclassDefinition).name ?? "")).filter(Boolean).join(" / "),
    playerRace: String(record(character.race).baseName ?? record(character.race).fullName ?? record(character.race).name ?? ""),
    playerSubrace: String(record(character.race).subRaceShortName ?? record(character.race).subRaceName ?? ""),
    playerBackground: String(record(record(character.background).definition).name ?? record(character.background).name ?? ""),
    playerName: String(character.playerName ?? ""),
